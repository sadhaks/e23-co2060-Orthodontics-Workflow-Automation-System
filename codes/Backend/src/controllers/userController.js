...
const deactivateUserAssignmentsSafely = async (userId) => {
  ...
  await query(
    `DELETE pa
     ...
     INNER JOIN patient_assignments inactive_pa
       ON inactive_pa.patient_id = pa.patient_id
      AND inactive_pa.user_id = pa.user_id
      AND inactive_pa.assignment_role = pa.assignment_role
      AND inactive_pa.active = FALSE
     WHERE pa.user_id = ?
       AND pa.active = TRUE`,
    [userId]
  );
